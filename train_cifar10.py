...
from utils.utils_loss import partial_loss2,semantic_loss
...
parser.add_argument('-dataset', help='specify a dataset', default='cifar10', type=str,required=False)
parser.add_argument('-n_class', help='the number of class', default=10, type=int)
...
parser.add_argument('-exp_name', default='test_cc', type=str)
#######
...
parser.add_argument('-n_map', help='the number of layer in map', default=2, type=int)
...
def main():
    ...
    ####
    loss_fn = partial_loss2(init_confidence)
    semantic_loss_fn=semantic_loss(args.sigma)
    model=get_model()
    optimizer = torch.optim.SGD(model.parameters(), args.lr,momentum=args.momentum,weight_decay=args.weight_decay)
    avg_train_acc,avg_test_acc,confidence_acc_list=train(log,model,optimizer,loss_fn,semantic_loss_fn,train_loader,test_loader,class_embedding)
...
def train(log,model,optimizer,loss_fn,semantic_loss_fn,train_loader,test_loader,class_embedding):
    train_acc_list = []
    test_acc_list = []
    confidence_acc_list = []
    for epoch in range(args.epoch):
        ...
        train_one_epoch(log, epoch, model, optimizer, loss_fn,semantic_loss_fn, train_loader, train_acc_list,confidence_acc_list,class_embedding)
    ...
    return avg_train_acc, avg_test_acc, confidence_acc_list

def train_one_epoch(log,epoch,model,optimizer,loss_fn,semantic_loss_fn,train_loader,train_acc_list,confidence_acc_list,class_embedding):
    model.train()
    ...
    for i,(images,labels,true_labels,index) in enumerate(train_loader):
        X,Y,ty,index = images.to(device), labels.to(device),true_labels.to(device), index.to(device)
        outputs,feature= model(X)
        loss1 = loss_fn(outputs,index)
        #######
        loss2=semantic_loss_fn(feature,class_embedding,loss_fn.confidence[index])
        loss = loss1 + args.loss_weight * loss2
        ####
        loss_fn.confidence_update(outputs,Y,index)
        ####
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        #######
        acc = accuracy(outputs,ty)[0]
        ...
        #########
    print('Train Epoch [{}]: lr:{:.4f}. Tr Acc:{:.2f}. Confidence Acc:{:.2f}.  loss_cls:{:.8f}. loss_alignment:{:.8f}'.format(epoch + 1, optimizer.param_groups[0]['lr'], acc_train.avg, acc_confidence.avg, loss_cls.avg,loss_cos.avg))
    log.write('Train Epoch [{}]: lr:{:.4f}. Tr Acc:{:.2f}. Confidence Acc:{:.2f}. loss_cls:{:.8f}. loss_alignment:{:.8f}\n'.format(epoch + 1, optimizer.param_groups[0]['lr'], acc_train.avg, acc_confidence.avg, loss_cls.avg,loss_cos.avg))
    log.flush()
    train_acc_list.extend([acc_train.avg.item()])
    confidence_acc_list.extend([acc_confidence.avg.item()])
